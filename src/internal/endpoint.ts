...
const ENDPOINT_RE = /^((?:background$)|devtools|popup|options|content-script|window|sidepanel|offscreen)(?:@(\d+)(?:\.(\d+))?)?$/
...
export const formatEndpoint = ({ context, tabId, frameId }: Endpoint): string => {
  if (['background', 'popup', 'options', 'sidepanel', 'offscreen'].includes(context))
    return context
